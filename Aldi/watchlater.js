...
function loadWatchlater() {
  ...
  const watchlater =
    JSON.parse(localStorage.getItem(`${loggedInUser}_watchedlater`)) || [];
  console.log('Loaded watch later for user:', loggedInUser, watchlater);

  if (watchlater.length === 0) {
    document.getElementById('watchlater-list').innerHTML =
      '<p>No watch later yet.</p>';
    return;
  }

  watchlater.forEach((item) => {
    fetchMovieDetail(item);
  });
}
...
function fetchMovieDetail(watch) {
  ...
  fetch(url)
    ...
    .catch((error) =>
      console.error('Error fetching watch later movie:', error)
    );
}

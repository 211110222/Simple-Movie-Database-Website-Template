...
function toggleWatchlater() {
  const loggedInUser = localStorage.getItem('loggedInUser');
  if (!loggedInUser) {
      alert('Please log in to watch later movies.');
      return;
  }

  const watchlater = JSON.parse(localStorage.getItem(`${loggedInUser}_watchedlater`)) || [];
  const watchlaterEntry = { id: String(movieId), type: mediaType };
  const existingIndex = watchlater.findIndex((entry) => entry.id === String(movieId));

  if (existingIndex !== -1) {
      watchlater.splice(existingIndex, 1);
      alert('Removed from watch later.');
  } else {
      watchlater.push(watchlaterEntry);
      alert('Add to watch later.');
  }
  localStorage.setItem(`${loggedInUser}_watchedlater`, JSON.stringify(watchlater));
...
function updateWatchlaterButton() {
  ...
  const watchlater = JSON.parse(localStorage.getItem(`${loggedInUser}_watchedlater`)) || [];
  const watchlaterButton = document.getElementById('watchlater-button');

  if (watchlater.find((entry) => entry.id === movieId)) {
      watchlaterButton.textContent = 'Remove from Watch Later';
      watchlaterButton.classList.add('Unwatchedlater');
  } else {
      watchlaterButton.textContent = 'Add to Watch Later';
      watchlaterButton.classList.remove('Watchedlater');
  }
}
